...
public abstract class ClasspathHelper {
    private static Logger log = LogUtil.findLogger(ClasspathHelper.class);
    ...
    public static Collection<URL> forResource(String resourceName, ClassLoader... classLoaders) {
        ...
        for (ClassLoader classLoader : loaders) {
            try {
                ...
                log.error("error getting resources for " + resourceName, e);
            }
        }
    ...
    public static URL forClass(Class<?> aClass, ClassLoader... classLoaders) {
        final ClassLoader[] loaders = classLoaders(classLoaders);
        final String resourceName = FileUtils.classPath2SystemPath(aClass.getName()) + ".class";
        for (ClassLoader classLoader : loaders) {
            try {
    ...
    public static Collection<URL> forManifest(final URL url) {
        ...
        try {
            ...
            log.debug(" don't do anything, we're going on the assumption it is a jar, which could be wrong\n", e);
        }
        return distinctUrls(result);
    }
    ...
    private static Collection<URL> distinctUrls(Collection<URL> urls) {
        Set<String> existUrl = new HashSet<>(urls.size());
        return urls.stream().filter(url -> {
            String externalForm = url.toExternalForm();
            boolean isNew = !existUrl.contains(externalForm);
            if (isNew) {
                existUrl.add(externalForm);
            }
            return isNew;
