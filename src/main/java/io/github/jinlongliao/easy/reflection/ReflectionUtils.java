...
import io.github.jinlongliao.easy.reflection.util.LogicUtils;
...
import static io.github.jinlongliao.easy.reflection.util.LogicUtils.filter;
 ...
 * > Set&lt;?> result = getAllXXX(type/s, withYYY) </pre>
...
public abstract class ReflectionUtils {
    public static org.slf4j.Logger log = LogUtil.findLogger(ReflectionUtils.class);
    ...
    public static Set<Class<?>> getAllSuperTypes(final Class<?> type, Predicate<? super Class<?>>... predicates) {
        ...
        return LogicUtils.filter(result, predicates);
    }
    ...
    public static Set<Method> getMethods(Class<?> t, Predicate<? super Method>... predicates) {
        return LogicUtils.filter(t.isInterface() ? t.getMethods() : t.getDeclaredMethods(), predicates);
    }
    ...
    public static Set<Constructor> getConstructors(Class<?> t, Predicate<? super Constructor>... predicates) {
        return LogicUtils.filter(t.getDeclaredConstructors(), predicates);
    }
    ...
    public static Set<Field> getFields(Class<?> type, Predicate<? super Field>... predicates) {
        return LogicUtils.filter(type.getDeclaredFields(), predicates);
    }
    ...
    public static <T extends AnnotatedElement> Set<Annotation> getAnnotations(T type, Predicate<Annotation>... predicates) {
        return LogicUtils.filter(type.getDeclaredAnnotations(), predicates);
    }
    ...
    public static <T extends AnnotatedElement> Set<T> getAll(final Set<T> elements, Predicate<? super T>... predicates) {
        return LogicUtils.filter(elements, predicates);
    }
    ...
    public static Class<?> forName(String typeName, ClassLoader... classLoaders) {
        if (getPrimitiveNames().contains(typeName)) {
            ...
            for (ReflectionsException reflectionsException : reflectionsExceptions) {
                log.warn("could not get type for name " + typeName + " from any class loader",
